                        ...
                        <tbody>
                            {transactions && transactions.map((t, index) => (
                                <tr key={index} className="p-2 text-xs text-left border-b">
                                    ...
                                    {t.type === "buy" ? <td className="px-2 text-left text-base-12 font-semibold">{ethers.formatEther(t.amountToken)}</td> : <td className="px-2 text-left font-semibold text-base-21">{ethers.formatEther(t.amountToken)}</td>}
                                    {t.type === "buy" ? <td className="px-2 text-left text-base-12 font-semibold">{parseFloat(ethers.formatEther(t.amountETH)).toFixed(5)}</td> : <td className="px-2 text-left font-semibold text-base-21">{parseFloat(ethers.formatEther(t.amountETH)).toFixed(5)}</td>}
                                    {t.type === "buy" ? <td className="px-2 text-left text-base-12 font-semibold">{parseFloat(ethers.formatEther(t.lastTokenPrice)).toFixed(9)}</td> : <td className="px-2 text-left font-semibold text-base-21">{parseFloat(ethers.formatEther(t.lastTokenPrice)).toFixed(9)}</td>}
                                </tr>
                            ))}
                        </tbody>
                    </table>
